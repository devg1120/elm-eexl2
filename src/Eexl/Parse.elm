module Eexl.Parse exposing (parse, parse2, parse3, parse4)
...
parse2 : Context -> String -> String
parse2 context string =
    deleteComment string

parse3 : Context -> String -> String
parse3 context string =
    --run (script context) (string ++ ";")
    let
      result   = parse4 context string
    in
    case result of
       Ok  stringList ->
                   List.foldl  (\x a -> x ++ a) "" stringList
       Err _  ->
                   "ERR-"
parse4 : Context -> String -> Result (List DeadEnd) (List Stmt)
parse4 context string =
    --run (script context) (string ++ ";")
    let
      string2 = deleteComment string
    in
